...
| **ب** | **تسجيل الرابط** — J&T لازم تعرف تبعت على **فين** | 🟢 **من البوابة بإيد المالك** | ✅ اتعمل 21 سبتمبر · الحالة `Applying` (مراجعة 1–2 يوم عمل) |

#### 🔴 تصحيح لقياس غلط — التسجيل مش إجراء إداري

كتبت هنا «مفيش أي باراميتر `url` في أي endpoint، يعني إجراء عندهم». **الجملة
الأولى صح والاستنتاج غلط.** فيه شاشتين في البوابة بيعملوا الموضوع:

1. **`Sandbox Mgt.` ← تبويب `Register the category`** — حط الرابط في خانة
   `Interwaybill address` ودوس **`Debugging`**. الشاشة بتقول حرفياً:
   «only after succeed **three or more times** can you submit an application
   for a formal environment».
2. **`Interface Mgt.`** ← `Application` جنب الـinterface ← الرابط ← `Save`
   ← الحالة تبقى `Applying` ← مراجعة 1–2 يوم عمل.

أسماء الصفوف عندهم بالصيني: `物流轨迹回传` = تتبع (`/trace`) ·
`订单状态回传` = حالة الأوردر (`/order`).
⚠️ صف حالة الأوردر كان متحطّ فيه `webhook.site/…` (رابط تجريبي) — اتغيّر.

**الدرس (امتداد لدرس 48):** التوثيق مش المكان الوحيد اللي فيه الإجابة —
**واجهة التنفيذ كمان**. أنا قريت التوثيق كله ومابصّيتش على البوابة.
...
### 🟢 وأول callbacks حقيقية وصلت (21 سبتمبر 17:25–17:26)

| الوقت (القاهرة) | النوع | البوليصة | `digest_ok` | `apply_note` |
|---|---|---|---|---|
| 17:25:20 | `trace` | `UT0001359848461` | ✅ **true** | `order_not_found` |
| 17:26:07 | `order` | `JT4000073746531` | ✅ **true** | `order_not_found` |

**ده قفل أكبر مخاطرة كانت فاضلة**: `jt-status` بترفض أي نداء مش موقّع بـ`401`،
وماكانش فيه أي دليل إن J&T بتوقّع الـcallbacks بمفتاحنا. دلوقتي اتأكد.
`order_not_found` **صح** — دي بوالص تجريبية مش بتاعتنا.
⚠️ صف واحد لكل نوع رغم تكرار الضغط: **حارس التكرار** بيرد `code 1` من غير ما
يكرر الصف، فالمحاولات نجحت عندهم واتسجّلت مرة عندنا. مقصود.

🔴 **بس دي بيانات المثال من التوثيق بالحرف** (`JT4000073746531` · `test0040` ·
南京雨花台板桥网点). بتثبت إن الأنبوبة شغالة — **مابتثبتش** شكل الحمولة
المصرية الحقيقية (درس 26).

🔴 **و`scanTypeCode` مش موجود في الحمولة أصلاً** رغم إن التوثيق بيقوله إجباري
(`Y`) — اللي موجود `scanType` كنص بس (`"发件扫描"`). فالأرقام 1–15 في صفحة
التوثيق هي **عُقد الاشتراك** (`traceNode`) مش اللي بيرجع في الـcallback.
`jt_status_map` بيفضل **فاضي** لحد أول حمولة من شحنة مصرية حقيقية.

🔴 **التحقق النهائي = أول callback على بوليصة `JEG…` بتاعتنا** (مش بوليصة المثال).
الاستعلام:
