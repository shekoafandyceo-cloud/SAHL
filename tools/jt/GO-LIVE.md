...
> **قالب الرفع الجماعي من البوابة (21 سبتمبر — المالك بعته):** `Download_template.xls`
> فيه **مستويين بس**: 26 محافظة و237 مدينة بأسماء J&T (named ranges في `Sheet2`) —
> محفوظين في `docs/pca-prov-city-template-2023.json`. **المنطقة (`*Arrival area`)
> عمود مطلوب في القالب ومفيش قايمة ليها في الملف**، و`jt_pca.area` `NOT NULL`
> فالملف مايتحمّلش في الجدول. ⚠️ والملف قديم (آخر حفظ أكتوبر 2023) وناقص:
> «الزقازيق» مش تحت الشرقية مع إن الـSandbox استخدمتها. اللي أكّده الملف:
> المرسل `القاهرة` → `السلام` ✓ · مستلم `17170`: `الجيزة` → `مدينة السادس من أكتوبر` ✓
> — **الفاضل اسم المنطقة تحتها من البوابة**. ومحافظات اللاندنج (30 يوم) كلها
> بتتطابق بالتطبيع ما عدا «شمال سيناء» (أوردر واحد — مش في نطاق J&T أصلاً).
...
5. ✅ v56 اترفعت (21 سبتمبر) واتأكدت بالبايت من الحي — كل ملفات J&T مطابقة و`/billing` 404.
