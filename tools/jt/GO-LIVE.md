...
العناوين من قالب البوابة، وكود الفرز بييجي في رد `addOrder`، والتكلفة المؤكدة من
`settlementReturn`.
...
7. ✅ v57 اترفعت · ✅ **`order/addOrder` اتفعّل** (بند 2ج) · ✅ `jt_production_enabled='true'` مفتوح.
   ⏳ **الفاضل: اختيار الأوردر + الضغطة.** ⚠️ `17170` **خرج** — اتحوّل `bosta_assigned` بإيد `ebrahim`
   (21 سبتمبر 08:45 UTC) وهو من غير بوليصة، فحارس `bad_status` في `jt-ship` بيرفضه. يا يترجّع
   «مؤكد» من اللوحة يا يتاخد أوردر تاني مؤكد من غير بوليصة.
