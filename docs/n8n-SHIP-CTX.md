...
## 0-ب) مراجعة تانية — 7 أغسطس (النسخة المنشورة `18137b47`)

> المالك خلّص خطوات تانية ونشر. **الحالة دلوقتي:**

### اتعمل ✅ (جديد)
- `get_order_details` موجودة **وبفلتر `tenant_id` + `id` مع بعض** + `alwaysOutputData` ✓ — البند الأحمر اتقفل
- `get_tenant_details` الفلتر اتملى (`body.tenant_id`) + `alwaysOutputData` ✓
- `SHIPT CTX` بتدمج المسارين بـtry/catch على أسماء النودز — صح ✓

### لسه ناقص ❌
1. **`Send a text message4/5/8`** لسه على `$('Update Order → confirmed')` (9 إشارات)
   — وكمان بتقرا حقول قديمة من عصر الشيتس (`PHONE` · `row_number` · `ADDRESS`)
   و**تلجرام بـchatId متسمّر لجروب عتبة** (`-5130323197`). على مسار اللوحة بتقع
   خالص (نود مش متنفذة) → فشل العنوان = **صمت كامل**.
2. **مسار `Webhook2` لسه `ordercreate`** — قابل للتخمين، وبينشئ شحنات بفلوس.
3. حارس `tracking_no` جوه الفلو — لسه مفيش (هيتغطى في الـEdge Function،
   بس دفاع الطبقتين أفضل).

### 🐛 باجات اتكشفت في المراجعة دي (مش من قايمة الستة)
- 🔴 **`Update a row2` بتقرا `$('BOSTA API')` وهي على فرع `BOSTA API1`**
  (مسار «المدينة اتحلت من الجدول التاني» — `If7 true`). النود المرجعية مش
  متنفذة على الفرع ده → التعبيرة ترمي → **الشحنة اتعملت ببوسطة فعلاً
  والحالة ورقم التتبع عمرهم ما بيتكتبوا**. الإصلاح: تعبيرة واحدة
  (`BOSTA API` → `BOSTA API1`) في النود.
- 🟠 حقلا `customer_ranking` في `Update a row`/`Update a row2` بيقروا
  `$json.data.deliveries[0].receiver.ranking` — لو بحث الـranking فشل
  (النود عليها `continueRegularOutput` فبتعدّي error item)، التعبيرة ترمي
  والكتابة كلها تفشل **بعد إنشاء الشحنة**. الإصلاح: optional chaining:
  `{{ $json.data?.deliveries?.[0]?.receiver?.ranking ?? null }}`.

### 📏 قياس حي (7 أغسطس)
الفجوة بين «confirmed» وكتابة `BOSTA AUTO` = **12–30 ثانية** بثبات
(10 أوردرات متعاقبة، الأحدث نفس اليوم) — يعني زرار اللوحة يقدر يستنى
النتيجة الحقيقية بـpolling قصير. والحالة بتتكتب **في آخر نود بس بعد نجاح
`BOSTA API`** — مفيش سيناريو «حالة شحن من غير شحنة» في التصميم الحالي؛
المخاطرة المعكوسة (شحنة من غير حالة) هي الموجودة، بالباجين اللي فوق.
